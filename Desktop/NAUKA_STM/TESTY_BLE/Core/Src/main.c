...
void process_uart1_rx(void); // Zmiana: nie przekazujemy char*
...
int main(void)
{
  ...
  HAL_Delay(1000); // Dodaj opóźnienie po inicjalizacji
  /* USER CODE END 2 */
  ...
  while (1)
  {
      if (isStarted)
      {
          ...
          if (HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY) == HAL_OK)
          {
              ...
              current_mA = adcValue / 10; // skaluj ADC do symulowanej wartości prądu (np. 0–409 mA)
          }
          HAL_ADC_Stop(&hadc1);

          distance += 1; // co 500ms +1m np.
          if (battery > 0) battery--;

          char msg[64];
          snprintf(msg, sizeof(msg), "%s,%d,%lu,%lu\n",
...
{
  if (GPIO_Pin == GPIO_PIN_13)  // dopasuj do pinu!
  {
    isStarted = !isStarted;  // zmiana stanu jazdy
    HAL_GPIO_TogglePin(LEDZIK_GPIO_Port, LEDZIK_Pin);  // opcjonalnie LED
  }
}

void process_uart1_rx(void)
{
    if (strstr(rx1_buffer, "START"))
    {
        isStarted = true;
    }
    else if (strstr(rx1_buffer, "STOP"))
    {
        isStarted = false;
    }
}
